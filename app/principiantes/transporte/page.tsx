...
const TRANSPORT_FILES = [
  'Airplane-chair-sofa.png',
  'Bicycle-chair-sofa.png',
  'Boat-chair-sofa.png',
  'Bus-chair-sofa.png',
  'Car-chair-sofa.png',
  'Helicopter-chair-sofa.png',
  'Hot_air_ballon-chair-sofa.png',
  'Train-chair-sofa.png',
...
const toAudioName = (value: string) => {
  ...
  const fixed = normalized.replace(/\bballon\b/i, 'balloon');
  const label = fixed
    .split(' ')
    .filter(Boolean)
    .map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase())
    .join(' ');
  return `Transportation_${label}`;
};
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        if (!meRes.ok) {
          router.replace(`/auth/login?next=${encodeURIComponent('/principiantes/transporte')}`);
          return;
        }
        setAuthReady(true);
      } catch {
      ...
      <div className="student-layout">
        ...
        <div className="student-content">
          <section className="quiz-header">
            <div>
              <h1>Transporte</h1>
              <p className="ui-muted">Selecciona el transporte correcto en cada tarjeta.</p>
            </div>
            <button
              type="button"
              className="ui-button"
          ...
          {!finished && current && (
            <section className="quiz-question">
              ...
              >
                ...
                <img
                  src={current.image}
                  alt={`Transporte ${current.correct}`}
