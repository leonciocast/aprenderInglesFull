...
const WEATHER_FILES = [
  ...
  'Foggy-mini-camp.png',
  'Hot-mini-camp.png',
  'Partly_cloudy-mini-camp.png',
  'Rainy-mini-camp.png',
  'Snow-mini-camp.png',
  'Stormy-mini-camp.png',
  'Sunny-mini-camp.png',
  'Windy-mini-camp.png',
];

const WEATHER_STOPWORDS = ['mini', 'camp'];
const IMAGE_BASE = '/uploader/image/Weather';
const AUDIO_BASE = '/uploader/Audio/Weather';
...
const toDisplayLabel = (value: string) => {
  ...
  const mapped = weatherMap[normalized] || value.replace(/_/g, ' ');
  return mapped
    .split(' ')
    .filter(Boolean)
...
function buildQuestions(): Question[] {
  const correctList = WEATHER_FILES.map(file => {
    const base = file.replace(/\.[^.]+$/, '');
    const parts = base
      .split('-')
      .filter(Boolean)
      .filter(part => !WEATHER_STOPWORDS.includes(part.toLowerCase()));
    return parts[0];
  }).filter(Boolean);

  return WEATHER_FILES.map(file => {
    const base = file.replace(/\.[^.]+$/, '');
    const parts = base
      .split('-')
      .filter(Boolean)
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        if (!meRes.ok) {
          router.replace(`/auth/login?next=${encodeURIComponent('/principiantes/clima')}`);
          return;
        }
        setAuthReady(true);
      } catch {
      ...
      <div className="student-layout">
        ...
        <div className="student-content">
          <section className="quiz-header">
            <div>
              <h1>Clima</h1>
              <p className="ui-muted">Selecciona el clima correcto en cada tarjeta.</p>
            </div>
            <button
              type="button"
              className="ui-button"
