...
const COUNTRY_FILES = [
  'Argentina-pants-skirt.png',
  'Brazil-pants-skirt.png',
  'Canada-pants-skirt.png',
  'France-pants-skirt.png',
  'Germany-pants-skirt.png',
  'Italy-pants-skirt.png',
  'Mexico-pants-skirt.png',
  'United_kingdom-pants-skirt.png',
  'United_states-pants-skirt.png',
];

const COUNTRY_STOPWORDS = ['pants', 'skirt'];
const IMAGE_BASE = '/uploader/image/Pais';
const AUDIO_BASE = '/uploader/Audio/Paises';
...
function parseOptions(file: string) {
  ...
  const parts = base
    .split('-')
    .filter(Boolean)
    .filter(part => !COUNTRY_STOPWORDS.includes(part.toLowerCase()));
  const [correct, alt1, alt2] = parts;
  return { base, correct, options: [correct, alt1, alt2].filter(Boolean) };
}

function buildQuestions(): Question[] {
  const correctList = COUNTRY_FILES.map(file => {
    const base = file.replace(/\.[^.]+$/, '');
    const parts = base
      .split('-')
      .filter(Boolean)
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        if (!meRes.ok) {
          router.replace(`/auth/login?next=${encodeURIComponent('/principiantes/paises')}`);
          return;
        }
        setAuthReady(true);
      } catch {
      ...
      <div className="student-layout">
        ...
        <div className="student-content">
          <section className="quiz-header">
            <div>
              <h1>Países</h1>
              <p className="ui-muted">Selecciona el país correcto en cada tarjeta.</p>
            </div>
            <button
              type="button"
              className="ui-button"
