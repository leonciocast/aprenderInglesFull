...
const AIRPORT_FILES = [
  'Baggage_claim-mini-camp.png',
  'Boarding_pass-mini-camp.png',
  'Check_in-mini-camp.png',
  'Passport-mini-camp.png',
  'Security-mini-camp.png',
  'Taxi-mini-camp.png',
];

const IMAGE_BASE = '/uploader/image/Airport';
const AUDIO_BASE = '/uploader/Audio/Airport';
...
function buildQuestions(): Question[] {
  const bases = AIRPORT_FILES.map(file => file.replace(/\.[^.]+$/, ''));
  const correctList = bases.map(base => base.split('-').filter(Boolean)[0]).filter(Boolean);

  return AIRPORT_FILES.map(file => {
          ...
          {!finished && current && (
            <section className="quiz-question">
              ...
              >
                ...
                <img
                  src={current.image}
                  alt={`Aeropuerto ${current.correct}`}
