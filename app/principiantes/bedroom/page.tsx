...
const BEDROOM_FILES = [
  'alarm_clock-bookshelf-pillow.png',
  'bed-bookshelf-pillow.png',
  'bookshelf-pillow-window.png',
  'clothes-bookshelf-pillow.png',
  'dresser-bookshelf-pillow.png',
  'lamp-bookshelf-pillow.png',
  'nightstand-bookshelf-pillow.png',
  'pillow-bookshelf-pillow.png',
  'window-bookshelf-pillow.png',
];

const IMAGE_BASE = '/uploader/image/Bedroom';
const AUDIO_BASE = '/uploader/Audio/Bedroom';
...
function buildQuestions(): Question[] {
  const correctList = BEDROOM_FILES.map(file => {
    const base = file.replace(/\.[^.]+$/, '');
    return base.split('-').filter(Boolean)[0];
  }).filter(Boolean);

  return BEDROOM_FILES.map(file => {
    const base = file.replace(/\.[^.]+$/, '');
...
export default function BedroomPage() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        if (!meRes.ok) {
          router.replace(`/auth/login?next=${encodeURIComponent('/principiantes/bedroom')}`);
          return;
        }
        setAuthReady(true);
      } catch {
  ...
  const playAudio = (item: string) => {
    ...
    if (!audio) return;
    const label = audioName.replace('Bedroom_', '');
    const variants = [
      audioName,
      `Bedroom__${label}`,
      `Bedroom_${label}`,
      ...
      <div className="student-layout">
        ...
        <div className="student-content">
          <section className="quiz-header">
            <div>
              <h1>Dormitorio</h1>
              <p className="ui-muted">Selecciona el objeto correcto en cada tarjeta.</p>
            </div>
            <button
              type="button"
              className="ui-button"
          ...
          {!finished && current && (
            <section className="quiz-question">
              <h3>¿Qué objeto del dormitorio es?</h3>
              <div
                style={{
                  background: '#fff',
                  borderRadius: 18,
              ...
              >
                ...
                <img
                  src={current.image}
                  alt={`Objeto del dormitorio ${current.correct}`}
