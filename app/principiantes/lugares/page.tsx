...
const PLACE_FILES = [
  'Bank-chair-sofa.png',
  'Beach-chair-sofa.png',
  'Gas_station-chair-sofa.png',
  'Hospital-chair-sofa.png',
  'Museum-chair-sofa.png',
  'Park-chair-sofa.png',
  'Restaurant-chair-sofa.png',
  'School-chair-sofa.png',
  'Supermarket-chair-sofa.png',
];

const PLACE_STOPWORDS = ['chair', 'sofa'];
const IMAGE_BASE = '/uploader/image/Places';
const AUDIO_BASE = '/uploader/Audio/Places';
...
function buildQuestions(): Question[] {
  const correctList = PLACE_FILES.map(file => {
    const base = file.replace(/\.[^.]+$/, '');
    const parts = base
      .split('-')
      .filter(Boolean)
      .filter(part => !PLACE_STOPWORDS.includes(part.toLowerCase()));
    return parts[0];
  }).filter(Boolean);

  return PLACE_FILES.map(file => {
    const base = file.replace(/\.[^.]+$/, '');
    const parts = base
      .split('-')
      .filter(Boolean)
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        if (!meRes.ok) {
          router.replace(`/auth/login?next=${encodeURIComponent('/principiantes/lugares')}`);
          return;
        }
        setAuthReady(true);
      } catch {
      ...
      <div className="student-layout">
        ...
        <div className="student-content">
          <section className="quiz-header">
            <div>
              <h1>Lugares</h1>
              <p className="ui-muted">Selecciona el lugar correcto en cada tarjeta.</p>
            </div>
            <button
              type="button"
              className="ui-button"
