...
const COLOR_FILES = [
  'Black-chair-sofa.png',
  'Blue-chair-sofa.png',
  'Brown-chair-sofa.png',
  'Green-chair-sofa.png',
  'Orange-chair-sofa.png',
  'Pink-chair-sofa.png',
  'Red-chair-sofa.png',
  'White-chair-sofa.png',
  'Yellow-chair-sofa.png',
];

const COLOR_STOPWORDS = ['chair', 'sofa'];
const IMAGE_BASE = '/uploader/image/Colors';
const AUDIO_BASE = '/uploader/Audio/Colors';
...
function buildQuestions(): Question[] {
  const correctList = COLOR_FILES.map(file => {
    const base = file.replace(/\.[^.]+$/, '');
    const parts = base
      .split('-')
      .filter(Boolean)
      .filter(part => !COLOR_STOPWORDS.includes(part.toLowerCase()));
    return parts[0];
  }).filter(Boolean);

  return COLOR_FILES.map(file => {
    const base = file.replace(/\.[^.]+$/, '');
    const parts = base
      .split('-')
      .filter(Boolean)
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        if (!meRes.ok) {
          router.replace(`/auth/login?next=${encodeURIComponent('/principiantes/colores')}`);
          return;
        }
        setAuthReady(true);
      } catch {
      ...
      <div className="student-layout">
        ...
        <div className="student-content">
          <section className="quiz-header">
            <div>
              <h1>Colores</h1>
              <p className="ui-muted">Selecciona el color correcto en cada tarjeta.</p>
            </div>
            <button
              type="button"
              className="ui-button"
