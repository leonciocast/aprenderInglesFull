...
function parseOptions(file: string) {
  ...
  const parts = base
    .split('-')
    .filter(Boolean)
    .filter(part => !MONTH_STOPWORDS.includes(part.toLowerCase()));
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        if (!meRes.ok) {
          router.replace(`/auth/login?next=${encodeURIComponent('/principiantes/meses')}`);
          return;
        }
        setAuthReady(true);
      } catch {
      ...
      <div className="student-layout">
        ...
        <div className="student-content">
          <section className="quiz-header">
            <div>
              <h1>Meses</h1>
              <p className="ui-muted">Selecciona el mes correcto en cada tarjeta.</p>
            </div>
            <button
              type="button"
              className="ui-button"
