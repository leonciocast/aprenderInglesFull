...
const WEEKDAY_FILES = [
  'domingo-febrero-marzo.png',
  'jueves-febrero-marzo.png',
  'lunes-febrero-marzo.png',
  'martes--febrero-marzo.png',
  'miércoles-febrero-marzo.png',
  'sábado-febrero-marzo.png',
...
const toAudioName = (value: string) => {
  ...
  const mapped = weekdayMap[normalized] || value;
  return `Weekdays_${mapped.charAt(0).toUpperCase()}${mapped.slice(1)}`;
};
...
function parseOptions(file: string) {
  ...
  const parts = base
    .split('-')
    .filter(Boolean)
    .filter(part => !WEEKDAY_STOPWORDS.includes(part.toLowerCase()));
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        if (!meRes.ok) {
          router.replace(`/auth/login?next=${encodeURIComponent('/principiantes/dias')}`);
          return;
        }
        setAuthReady(true);
      } catch {
        ...
        <div className="student-content">
          <section className="quiz-header">
            <div>
              ...
              <p className="ui-muted">Selecciona el día correcto en cada tarjeta.</p>
            </div>
            <button
              type="button"
              className="ui-button"
