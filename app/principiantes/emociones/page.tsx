...
const EMOTION_FILES = [
  'angry-happy-sad.png',
  'confused-sad-angry.png',
  'cry-happy-sad.png',
  'happy-sad-angry.png',
  'nervous-happy-sad.png',
  'sad-happy-angry.png',
  'scared-sad-angry.png',
  'surprised-happy-angry.png',
  'tired-happy-sad.png',
];
...
function buildQuestions(): Question[] {
  const correctList = EMOTION_FILES.map(file => {
    const base = file.replace(/\.[^.]+$/, '');
    return base.split('-').filter(Boolean)[0];
  }).filter(Boolean);

  return EMOTION_FILES.map(file => {
    const base = file.replace(/\.[^.]+$/, '');
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        if (!meRes.ok) {
          router.replace(`/auth/login?next=${encodeURIComponent('/principiantes/emociones')}`);
          return;
        }
        setAuthReady(true);
      } catch {
      ...
      <div className="student-layout">
        ...
        <div className="student-content">
          <section className="quiz-header">
            <div>
              <h1>Emociones</h1>
              <p className="ui-muted">Adivina la emoción correcta en cada tarjeta.</p>
            </div>
            <button type="button" className="ui-button" onClick={() => router.push('/principiantes')}>
              Volver al libro
            </button>
          </section>
