...
const SPORT_FILES = [
  'Baseball-mini-camp.png',
  'Basketball-mini-camp.png',
  'Cycling-mini-camp.png',
  'Golf-mini-camp.png',
  'Running-mini-camp.png',
  'Soccer-mini-camp.png',
  'Swimming-mini-camp.png',
  'Tennis-mini-camp.png',
  'Weightlifting-mini-camp.png',
];

const IMAGE_BASE = '/uploader/image/Sport';
const AUDIO_BASE = '/uploader/Audio/Sports';
...
function parseOptions(file: string) {
  ...
  const parts = base.split('-').filter(Boolean).filter(part => {
    const normalized = part.toLowerCase();
    return normalized !== 'mini' && normalized !== 'camp';
  });
...
function buildQuestions(): Question[] {
  const bases = SPORT_FILES.map(file => file.replace(/\.[^.]+$/, ''));
  const correctList = bases
    .map(base => base.split('-').filter(Boolean)[0])
    .filter(Boolean);

  return SPORT_FILES.map(file => {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        if (!meRes.ok) {
          router.replace(`/auth/login?next=${encodeURIComponent('/principiantes/deportes')}`);
          return;
        }
        setAuthReady(true);
      } catch {
      ...
      <div className="student-layout">
        ...
        <div className="student-content">
          <section className="quiz-header">
            <div>
              <h1>Deportes</h1>
              <p className="ui-muted">Selecciona el deporte correcto en cada tarjeta.</p>
            </div>
            <button
              type="button"
              className="ui-button"
