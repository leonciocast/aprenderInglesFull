...
const SICKNESS_FILES = [
  'Back_pain-pants-skirt.png',
  'Cough-pants-skirt.png',
  'Fatigue-pants-skirt.png',
  'Fever-pants-skirt.png',
  'Headache-pants-skirt.png',
  'Injury-pants-skirt.png',
  'Sore_throat-pants-skirt.png',
  'Stomachache-pants-skirt.png',
  'Vomiting-pants-skirt.png',
];

const SICKNESS_STOPWORDS = ['pants', 'skirt'];
const IMAGE_BASE = '/uploader/image/Sickness';
const AUDIO_BASE = '/uploader/Audio/Sickness';
...
function parseOptions(file: string) {
  ...
  const parts = base
    .split('-')
    .filter(Boolean)
    .filter(part => !SICKNESS_STOPWORDS.includes(part.toLowerCase()));
  const [correct, alt1, alt2] = parts;
  return { base, correct, options: [correct, alt1, alt2].filter(Boolean) };
}

function buildQuestions(): Question[] {
  const correctList = SICKNESS_FILES.map(file => {
    const base = file.replace(/\.[^.]+$/, '');
    const parts = base
      .split('-')
      .filter(Boolean)
  ...
  return SICKNESS_FILES.map(file => {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        if (!meRes.ok) {
          router.replace(`/auth/login?next=${encodeURIComponent('/principiantes/enfermedades')}`);
          return;
        }
        setAuthReady(true);
      } catch {
      ...
      <div className="student-layout">
        ...
        <div className="student-content">
          <section className="quiz-header">
            <div>
              <h1>Enfermedades</h1>
              <p className="ui-muted">Selecciona la palabra correcta en cada tarjeta.</p>
            </div>
            <button
              type="button"
              className="ui-button"
          ...
          {!finished && current && (
            <section className="quiz-question">
              ...
              >
                ...
                <img
                  src={current.image}
                  alt={`Enfermedad ${current.correct}`}
