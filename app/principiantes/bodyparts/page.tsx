...
const AUDIO_BASE = '/uploader/Audio/BodyParts';

const toAudioName = (value: string) => {
  if (!value) return '';
  return `BodyPart_${value.charAt(0).toUpperCase()}${value.slice(1).toLowerCase()}`;
};
...
export default function BodyPartsPage() {
  ...
  const playAudio = (part: string) => {
    if (!part) return;
    const audioName = toAudioName(part);
    if (!audioName) return;
