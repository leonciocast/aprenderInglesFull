...
const KITCHEN_FILES = [
  'cutting_board-fork-spoon.png',
  'fork-spoon-spatula.png',
  'frying_pan-spoon-fork.png',
  'kitchen_shears-spoon-knife.png',
  'kitchen_tongs-spoon-fork.png',
  'knife-fork-spoon.png',
  'ladle-fork-spoon.png',
  'pot-spoon-fork.png',
  'rolling_pin-fork-spoon.png',
  'spatula-spoon-fork.png',
  'spoon-fork-knife.png',
  'whisk-spoon-fork.png',
];

const IMAGE_BASE = '/uploader/image/Kitchen';
const AUDIO_BASE = '/uploader/Audio/Kitchen';
...
function buildQuestions(): Question[] {
  const correctList = KITCHEN_FILES.map(file => {
    const base = file.replace(/\.[^.]+$/, '');
    return base.split('-').filter(Boolean)[0];
  }).filter(Boolean);

  return KITCHEN_FILES.map(file => {
    const base = file.replace(/\.[^.]+$/, '');
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        if (!meRes.ok) {
          router.replace(`/auth/login?next=${encodeURIComponent('/principiantes/cocina')}`);
          return;
        }
        setAuthReady(true);
      } catch {
      ...
      <div className="student-layout">
        ...
        <div className="student-content">
          <section className="quiz-header">
            <div>
              <h1>Cocina</h1>
              <p className="ui-muted">Selecciona el utensilio correcto en cada tarjeta.</p>
            </div>
            <button
              type="button"
              className="ui-button"
          ...
          {!finished && current && (
            <section className="quiz-question">
              ...
              >
                ...
                <img
                  src={current.image}
                  alt={`Objeto de cocina ${current.correct}`}
