...
const HOTEL_FILES = [
  'Bedroom-mini-camp.png',
  'Elevator-mini-camp.png',
  'Laundry-mini-camp.png',
  'Luggage-mini-camp.png',
  'Reception-mini-camp.png',
  'Restaurant-mini-camp.png',
  'Spa-mini-camp.png',
  'Swimming_pool-mini-chair.png',
  'Wi-fi-mini-camp.png',
];

const HOTEL_STOPWORDS = ['mini', 'camp'];
const IMAGE_BASE = '/uploader/image/Hotel';
const AUDIO_BASE = '/uploader/Audio/Hotel';
...
function parseOptions(file: string) {
  ...
  const normalizedBase = base.replace(/Wi-fi/gi, 'Wifi');
  const parts = base
    .replace(/Wi-fi/gi, 'Wifi')
    .split('-')
    .filter(Boolean)
    .filter(part => !HOTEL_STOPWORDS.includes(part.toLowerCase()));
  const [correct, alt1, alt2] = parts;
  return { base: normalizedBase, correct, options: [correct, alt1, alt2].filter(Boolean) };
}

function buildQuestions(): Question[] {
  const correctList = HOTEL_FILES.map(file => {
    const base = file.replace(/\.[^.]+$/, '');
    const parts = base
      .replace(/Wi-fi/gi, 'Wifi')
      .split('-')
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        if (!meRes.ok) {
          router.replace(`/auth/login?next=${encodeURIComponent('/principiantes/hotel')}`);
          return;
        }
        setAuthReady(true);
      } catch {
