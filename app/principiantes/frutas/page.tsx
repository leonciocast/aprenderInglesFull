...
const FRUIT_FILES = [
  'Apple-cacao-coco.png',
  'Banana-cacao-coco.png',
  'Cherry-cacao-coco.png',
  'Grape-cacao-coco.png',
  'Lemon-cacao-coco.png',
  'Orange-cacao-coco.png',
  'Pineapple-cacao-coco.png',
  'Strawberry-cacao-coco.png',
  'Watermelon-cacao-coco.png',
];

const FRUIT_STOPWORDS = ['cacao', 'coco'];
const IMAGE_BASE = '/uploader/image/Fruit';
const AUDIO_BASE = '/uploader/Audio/Fruits';
...
const toAudioName = (value: string) => {
  ...
  const mapped = fruitMap[normalized] || value.replace(/_/g, ' ');
  const label = mapped
    .split(' ')
    .filter(Boolean)
    .map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase())
    .join(' ');
  return `Fruits_${label}`;
};
...
function buildQuestions(): Question[] {
  const correctList = FRUIT_FILES.map(file => {
    const base = file.replace(/\.[^.]+$/, '');
    const parts = base
      .split('-')
      .filter(Boolean)
      .filter(part => !FRUIT_STOPWORDS.includes(part.toLowerCase()));
    return parts[0];
  }).filter(Boolean);

  return FRUIT_FILES.map(file => {
    const base = file.replace(/\.[^.]+$/, '');
    const parts = base
      .split('-')
      .filter(Boolean)
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        if (!meRes.ok) {
          router.replace(`/auth/login?next=${encodeURIComponent('/principiantes/frutas')}`);
          return;
        }
        setAuthReady(true);
      } catch {
      ...
      <div className="student-layout">
        ...
        <div className="student-content">
          <section className="quiz-header">
            <div>
              <h1>Frutas</h1>
              <p className="ui-muted">Selecciona la fruta correcta en cada tarjeta.</p>
            </div>
            <button
              type="button"
              className="ui-button"
