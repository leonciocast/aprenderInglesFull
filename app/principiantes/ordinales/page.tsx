...
const ORDINAL_ENTRIES: Array<[string[], string]> = [
  ...
  [['quinto'], 'Fifth'],
  [['sexto'], 'Sixth'],
  [['septimo', 'séptimo'], 'Seventh'],
  [['octavo'], 'Eighth'],
  [['noveno'], 'Ninth'],
  [['decimo', 'décimo'], 'Tenth'],
  [['undecimo', 'undécimo'], 'Eleventh'],
  [['duodecimo', 'duodécimo'], 'Twelfth'],
  [['decimotercero'], 'Thirteenth'],
  [['decimocuarto'], 'Fourteenth'],
  [['decimoquinto'], 'Fifteenth'],
  [['decimosexto'], 'Sixteenth'],
  [['decimoseptimo', 'decimoséptimo'], 'Seventeenth'],
  [['decimooctavo'], 'Eighteenth'],
  [['decimonoveno'], 'Nineteenth'],
  [['vigesimo', 'vigésimo', 'vigésimo'], 'Twentieth'],
  [
    ['vigesimoprimero', 'vegesimoprimero', 'vegésimoprimero', 'vegésimoPrimero'],
    'Twenty-first',
  ],
  [['trigesimo', 'trigésimo'], 'Thirtieth'],
  [['cuadragesimo', 'cuadragésimo'], 'Fortieth'],
  [['centesimo', 'centésimo'], 'One hundredth'],
];

const ordinalMap = ORDINAL_ENTRIES.reduce<Record<string, string>>((acc, [variants, value]) => {
  variants.forEach(variant => {
    acc[variant.normalize('NFC').toLowerCase()] = value;
  });
  return acc;
}, {});
...
export default function OrdinalesPage() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        if (!meRes.ok) {
          router.replace(`/auth/login?next=${encodeURIComponent('/principiantes/ordinales')}`);
          return;
        }
        setAuthReady(true);
      } catch {
  ...
  const playAudio = (value: string) => {
    ...
    if (!audio) return;
    const base = audioName.replace('Ordinal_numbers_', '');
    const variants = [
      audioName,
      `Ordinal_numbers_${base.toLowerCase()}`,
        ...
        <div className="student-content">
          <section className="quiz-header">
            <div>
              ...
              <p className="ui-muted">Selecciona el ordinal correcto en cada tarjeta.</p>
            </div>
            <button
              type="button"
              className="ui-button"
