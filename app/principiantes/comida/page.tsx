...
const FOOD_FILES = [
  'Burger-cacao-coco.png',
  'Cookies-cacao-coco.png',
  'Donut-cacao-coco.png',
  'Fries-cacao-coco.png',
  'Pasta-cacao-coco.png',
  'Pizza-cacao-coco.png',
  'Salad-cacao-coco.png',
  'Sandwich-cacao-coco.png',
  'Sushi-cacao-coco.png',
];

const FOOD_STOPWORDS = ['cacao', 'coco'];
const IMAGE_BASE = '/uploader/image/Food';
const AUDIO_BASE = '/uploader/Audio/Food';
...
function buildQuestions(): Question[] {
  const correctList = FOOD_FILES.map(file => {
    const base = file.replace(/\.[^.]+$/, '');
    const parts = base
      .split('-')
      .filter(Boolean)
      .filter(part => !FOOD_STOPWORDS.includes(part.toLowerCase()));
    return parts[0];
  }).filter(Boolean);

  return FOOD_FILES.map(file => {
    const base = file.replace(/\.[^.]+$/, '');
    const parts = base
      .split('-')
      .filter(Boolean)
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        if (!meRes.ok) {
          router.replace(`/auth/login?next=${encodeURIComponent('/principiantes/comida')}`);
          return;
        }
        setAuthReady(true);
      } catch {
      ...
      <div className="student-layout">
        ...
        <div className="student-content">
          <section className="quiz-header">
            <div>
              <h1>Comida</h1>
              <p className="ui-muted">Selecciona la comida correcta en cada tarjeta.</p>
            </div>
            <button
              type="button"
              className="ui-button"
