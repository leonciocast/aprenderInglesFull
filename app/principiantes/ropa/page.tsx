...
const CLOTHES_FILES = [
  ...
  'Pants-mini-camp.png',
  'Shoes-mini-camp.png',
  'Shorts-mini-camp.png',
  'Skirt-mini-camp.png',
  'Socks-mini-camp.png',
  'Sweater-mini-camp.png',
  'T_shirt-mini-camp.png',
];

const CLOTHES_STOPWORDS = ['mini', 'camp'];
const IMAGE_BASE = '/uploader/image/Clothes';
const AUDIO_BASE = '/uploader/Audio/Clothes';
...
const toAudioName = (value: string) => {
  ...
  const mapped = clothesMap[normalized] || value.replace(/_/g, ' ');
  const label = mapped
    .split(' ')
    .filter(Boolean)
    .map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase())
    .join(' ');
  return `Clothes_${label}`;
};
...
function buildQuestions(): Question[] {
  const correctList = CLOTHES_FILES.map(file => {
    const base = file.replace(/\.[^.]+$/, '');
    const parts = base
      .split('-')
      .filter(Boolean)
      .filter(part => !CLOTHES_STOPWORDS.includes(part.toLowerCase()));
    return parts[0];
  }).filter(Boolean);

  return CLOTHES_FILES.map(file => {
    const base = file.replace(/\.[^.]+$/, '');
    const parts = base
      .split('-')
      .filter(Boolean)
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        if (!meRes.ok) {
          router.replace(`/auth/login?next=${encodeURIComponent('/principiantes/ropa')}`);
          return;
        }
        setAuthReady(true);
      } catch {
      ...
      <div className="student-layout">
        ...
        <div className="student-content">
          <section className="quiz-header">
            <div>
              <h1>Ropa</h1>
              <p className="ui-muted">Selecciona la prenda correcta en cada tarjeta.</p>
            </div>
            <button
              type="button"
              className="ui-button"
