...
const PDF_FILENAME = 'Conexión Inmediata.pdf';
const downloadUrl = `/uploader/api/pdfs/download?file=${encodeURIComponent(PDF_FILENAME)}`;

export default function BonoConexionPage() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        if (!meRes.ok) {
          router.replace(`/auth/login?next=${encodeURIComponent('/bono/conexion')}`);
          return;
        }
        setStatus('ready');
      ...
      <main className="auth-page ui-shell ui-shell--light">
        <div className="auth-card ui-card">
          ...
          <button
            ...
            onClick={() => router.replace(`/auth/login?next=${encodeURIComponent('/bono/conexion')}`)}
          >
            Ir a iniciar sesión
          </button>
        </div>
